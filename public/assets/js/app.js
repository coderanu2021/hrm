...
(function () {
  ...
  navItems.forEach(item => {
    item.addEventListener('click', (e) => {
      ...
      // Only use the mockup SPA logic if the link is '#'
      if (href === '#' || href === '' || !href) {
          e.preventDefault();
  ...
  const pageMap = {
    'dashboard': { title: 'HR Admin Dashboard', subtitle: 'Monitor workforce activity and manage HR operations' },
    'directory': { title: 'Employee Directory', subtitle: 'Browse and manage all employees' },
  ...
  function showPage(page, _label) {
    if (!pageContent) return; // Prevent crash if pageContent is not found
