...
const pageMap = {
    // Recruitment
    'onboarding': { folder: 'recruitment', title: 'Onboarding', subtitle: 'Manage new employee onboarding workflows', type: 'kanban' },
    'job-postings': { folder: 'recruitment', title: 'Job Postings', subtitle: 'Create and manage open job positions', type: 'table', columns: ['Job Title', 'Department', 'Location', 'Status', 'Actions'] },
    'candidates': { folder: 'recruitment', title: 'Candidates', subtitle: 'Review and track applicants', type: 'kanban' },
    
    // Attendance
    'attendance-log': { folder: 'attendance', title: 'Attendance Log', subtitle: 'View and manage daily attendance records', type: 'table', columns: ['Employee', 'Date', 'Check In', 'Check Out', 'Status', 'Actions'] },
    'shifts': { folder: 'attendance', title: 'Shifts', subtitle: 'Schedule and manage employee shifts', type: 'calendar' },
    'corrections': { folder: 'attendance', title: 'Corrections', subtitle: 'Handle attendance and payroll corrections', type: 'table', columns: ['Employee', 'Type', 'Reason', 'Status', 'Actions'] },
    'geofencing': { folder: 'attendance', title: 'Geofencing', subtitle: 'Set location-based attendance boundaries', type: 'table', columns: ['Location Name', 'Coordinates', 'Radius', 'Status', 'Actions'] },
    
    // Employee
    'bulk-import': { folder: 'employee', title: 'Bulk Import', subtitle: 'Import employee data in bulk', type: 'form' },
    
    // Leaves
    'leave-management': { folder: 'leaves', title: 'Leave Management', subtitle: 'Review and manage employee leave requests', type: 'table', columns: ['Employee', 'Leave Type', 'Duration', 'Reason', 'Status', 'Actions'] },
    'leave-policies': { folder: 'leaves', title: 'Leave Policies', subtitle: 'Define and configure leave policies', type: 'table', columns: ['Policy Name', 'Accrual Rate', 'Max Balance', 'Status', 'Actions'] },
    'leave-calendar': { folder: 'leaves', title: 'Leave Calendar', subtitle: 'View team leave calendar', type: 'calendar' },
    'holiday-calendar': { folder: 'leaves', title: 'Holiday Calendar', subtitle: 'Manage company holidays', type: 'calendar' },
    'comp-off': { folder: 'leaves', title: 'Comp-Off', subtitle: 'Manage compensatory off balances', type: 'table', columns: ['Employee', 'Earned Hours', 'Used Hours', 'Balance', 'Status', 'Actions'] },
    
    // Performance
    'okrs': { folder: 'performance', title: 'OKRs', subtitle: 'Track objectives and key results', type: 'list' },
    'feedback': { folder: 'performance', title: '360 Feedback', subtitle: 'Manage multi-directional feedback cycles', type: 'list' },
    'review-cycles': { folder: 'performance', title: 'Review Cycles', subtitle: 'Configure and run performance reviews', type: 'table', columns: ['Cycle Name', 'Start Date', 'End Date', 'Status', 'Completion', 'Actions'] },
    'pips': { folder: 'performance', title: 'PIPs', subtitle: 'Manage performance improvement plans', type: 'table', columns: ['Employee', 'Manager', 'Start Date', 'End Date', 'Status', 'Actions'] },
    
    // Training
    'course-catalog': { folder: 'training', title: 'Course Catalog', subtitle: 'Browse and assign training courses', type: 'grid' },
    
    // Payroll
    'salary-structures': { folder: 'payroll', title: 'Salary Structures', subtitle: 'Configure compensation bands and structures', type: 'table', columns: ['Band Name', 'Min Salary', 'Max Salary', 'Status', 'Actions'] },
    'payroll-run': { folder: 'payroll', title: 'Payroll Run', subtitle: 'Process and manage payroll runs', type: 'table', columns: ['Cycle Month', 'Total Employees', 'Gross Pay', 'Status', 'Actions'] },
    
    // Reports
    'analytics': { folder: 'reports', title: 'Analytics', subtitle: 'Workforce insights and reports', type: 'dashboard' },
    'custom-reports': { folder: 'reports', title: 'Custom Reports', subtitle: 'Build and export custom HR reports', type: 'form' },
    
    // Settings
    'admin-settings': { folder: 'settings', title: 'Admin Settings', subtitle: 'Configure system and organization settings', type: 'form' },
    'role-management': { folder: 'settings', title: 'Role Management', subtitle: 'Manage user roles and permissions', type: 'table', columns: ['Role Name', 'Users Count', 'Permissions Level', 'Status', 'Actions'] },
    'audit-log': { folder: 'settings', title: 'Audit Log', subtitle: 'View all system activity logs', type: 'table', columns: ['Timestamp', 'User', 'Action', 'Module', 'IP Address'] }
};

function generateTable(columns) {
    if (!columns || columns.length === 0) columns = ['ID', 'Name', 'Date', 'Status', 'Actions'];
    
    let ths = columns.map((c, i) => {
        let align = i === columns.length - 1 ? 'text-align: right;' : 'text-align: left;';
        return `<th style="padding: 16px 24px; font-weight:600; color:var(--text-muted); font-size:12px; text-transform:uppercase; letter-spacing:0.5px; ${align}">${c}</th>`;
    }).join('\\n                ');
    
    let tds = columns.map((c, i) => {
        let isLast = i === columns.length - 1;
        let isStatus = c.toLowerCase().includes('status');
        let isFirst = i === 0;
        
        if (isLast) {
            return `<td style="padding: 16px 24px; text-align:right;">
        ...
        if (isStatus) {
            return `<td style="padding: 16px 24px;">
                   <span style="background: #ecfdf5; color: #059669; padding: 4px 12px; border-radius: 12px; font-size: 12px; font-weight: 600;">Active</span>
                </td>`;
        }
        
        if (isFirst) {
            return `<td style="padding: 16px 24px; color:var(--text-primary); font-weight:500;">
        ...
        return `<td style="padding: 16px 24px; color:var(--text-muted); font-size:14px;">Sample ${c}</td>`;
    }).join('\\n                ');
    ...
    return `
      ...
      <div class="header-actions" style="margin-bottom: 24px; display: flex; justify-content: space-between;">
        ...
        <button class="btn-primary" style="border-radius:8px;">
...
// Ensure base admin views directory exists
const baseViewsDir = 'F:/wamp64/www/hrm/resources/views/admin';

// Recreate controller content
...
for (const [key, info] of Object.entries(pageMap)) {
    ...
    const viewFile = path.join(folderDir, key + '.blade.php');
...
@endsection
`;
    // Write new grouped view file
    fs.writeFileSync(viewFile, viewContent.trim());
    
    // Add to controller
    controllerContent += `    public function ${methodName}() {\n        return view('admin.${info.folder}.${key}');\n    }\n\n`;
    
    // Add route using the grouped URI if possible, or just keep the same /admin/key for simplicity so layout links remain same
    // We will update the route URI to /admin/folder/key
    routesAdd += `Route::get('/admin/${info.folder}/${key}', [App\\Http\\Controllers\\AdminPagesController::class, '${methodName}'])->name('admin.${methodName}');\n`;
}

controllerContent += '}\n';
fs.writeFileSync('F:/wamp64/www/hrm/app/Http/Controllers/AdminPagesController.php', controllerContent);

// Optional: clean up old admin/pages folder
const oldPagesDir = path.join(baseViewsDir, 'pages');
if (fs.existsSync(oldPagesDir)) {
    fs.rmSync(oldPagesDir, { recursive: true, force: true });
}

console.log('Successfully generated GROUPED UIs with ENHANCED Table designs!');
